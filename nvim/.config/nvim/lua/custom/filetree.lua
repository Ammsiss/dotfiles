...
local function get_git_info()
    ...
    for _, line in ipairs(stdout_table) do
        if line:match("!!") then
            ...
            local user_path = vim.fn.getcwd()

            local git_path = git_root
            if vim.fs.dirname(rel_path) ~= "." then
                git_path = git_root .. "/" .. vim.fs.dirname(rel_path)
            end

            if user_path == git_path then
                output_map[vim.fs.basename(rel_path)] = "ignored"
            end
        end
    end

    if vim.fn.getcwd() == git_root then
        output_map[".git"] = "ignored"
    end
